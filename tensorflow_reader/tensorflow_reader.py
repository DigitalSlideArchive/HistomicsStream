...
class kwjpeg(Codec):
    ...
    def __init__(self, quality=100):
        ...
        assert (
            0 < self.quality <= 100 and isinstance(self.quality, int)
        )
        super().__init__()
    def encode(self, buf):
...
def readExample(devices, strategy):
    ...
    header = {
      'slide': ['TCGA-BH-A0BZ-01Z-00-DX1.45EB3E93-A871-49C6-9EAE-90D98AE01913' for x in range(len(allFiles))],
      'filename': allFiles,
    ...
    tiles = tiles.map(tf_ReadAndSplitChunk, **dataset_map_options).prefetch(tf.data.experimental.AUTOTUNE).unbatch()
    ...
    tiles = tiles.map(lambda tile, metadata: (tf.cast(tf.image.resize(tile, [224, 224]), tf.float32), metadata),
                      **dataset_map_options)
    tiles = tiles.map(lambda tile, metadata: (tf.keras.applications.resnet_v2.preprocess_input(tile), metadata),
                      **dataset_map_options)
...
def py_ReadChunk(filenameIn, level, x, y, w, h):
    ...
    if re.compile(r'\.svs$').search(filename):
        if True:               # Should be True!!!
            ...
            chunk = source_group[format(level.numpy())][y.numpy():(y.numpy()+h.numpy()), x.numpy():(x.numpy()+w.numpy()), :]
        else:
        ...
        if using_zarr_jpeg_package:
            ...
            chunk = np.transpose(source_group[format(level.numpy())][:, x.numpy():(x.numpy()+w.numpy()), y.numpy():(y.numpy()+h.numpy())])
        else:
        ...
        pil_obj = Image.open(filename)
        chunk = np.asarray(pil_obj)[x.numpy():(x.numpy()+w.numpy()), y.numpy():(y.numpy()+h.numpy()), :]
...
def convertOpenSlideToChunks(svsFilename, chunksDirname, chunksType='jpeg', chunkFactor=8, tileSize=256, overlap=0, **kwargs):
